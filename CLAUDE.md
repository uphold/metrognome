...
- `.metrognome/screen-map.md` — per-repo navigation brain: launch, auth, and routes to each screen (committed with the app; secrets referenced by `$NAME`, never inlined)
- `.metrognome/secrets.local.json` — `$NAME` → value map for screen-map auth steps (gitignored, bootstrapped as `{}`)
...
- `.metrognome/.gitignore` — excludes `report.html`, `run-state.json`, and `secrets.local.json` (generated/local-only artifacts)
